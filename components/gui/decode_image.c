...
esp_err_t decode_image(uint16_t ***pixels, uint8_t *jpeg, int* height, int* width)
{
    char *work = NULL;
    int r;
    JDEC decoder;
    ...
    *width = 0;
    ...
    jd.outW = decoder.width;
    jd.outH = decoder.height;
    *height = decoder.height;
    *width = decoder.width;

    ESP_LOGD(TAG, "Read image: w=%d, h=%d", jd.outW, jd.outH);
    ...
    *pixels = calloc(jd.outH, sizeof(uint16_t *));
    if (*pixels == NULL) {
        ESP_LOGE(TAG, "Error allocating memory for lines");
        ret = ESP_ERR_NO_MEM;
        goto err;
    }
    for (int i = 0; i < jd.outH; i++) {
        (*pixels)[i] = malloc(jd.outW * sizeof(uint16_t));
        if ((*pixels)[i] == NULL) {
